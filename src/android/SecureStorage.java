...
public class SecureStorage extends CordovaPlugin {
    ...
    private volatile CallbackContext  initContext, secureDeviceContext, generateKeysContext, unlockCredentialsContext;
            ...
            if (initContext != null && !initContextRunning) {
                cordova.getThreadPool().execute(new Runnable() {
                    public void run() {
                        ...
                        try {
                            String alias = service2alias(INIT_SERVICE);
                            if (!RSA.isEntryAvailableLegacy(alias)) {
    ...
    public boolean execute(String action, CordovaArgs args, final CallbackContext callbackContext) throws JSONException {
		...
		if(Build.VERSION.SDK_INT >= 29) {
		    ...
		    if (!isDeviceSecure()) {
                Log.e(TAG, MSG_DEVICE_NOT_SECURE);
                callbackContext.error(MSG_DEVICE_NOT_SECURE);
            } else if (!RSA.isEntryAvailableLegacy(alias)) {
